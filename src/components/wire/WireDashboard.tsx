...
// Category counts are derived from the live initialStories prop, not a
// mock array — and use League vocabulary (CPL, Provincial, CanMNT...) to
// match what getWireFeed()/the newsWire schema actually return.
function buildCategories(stories: WireStory[]) {
  const leagues = ['CPL', 'NSL', 'Provincial', 'MLS', 'Abroad', 'Transfers', 'CanMNT', 'CanWNT'];
  return [
    { name: 'ALL DISPATCHES', count: stories.length },
    ...leagues.map((league) => ({
      name: league,
      count: stories.filter((s) => s.category === league).length,
    })),
  ];
}
...
// Placeholder prospect list — same demo-labelled status as the standings
// arrays in page.tsx. Not tied to Sanity; swap for a real scouting data
// source whenever that exists.
...
export default function WireDashboard({ initialStories, standings, nslStandings }: WireDashboardProps) {
  ...
  const topSectionIds = new Set([
    heroStory?.id,
    ...largeDispatches.map((s) => s.id),
    ...compactDispatches.map((s) => s.id),
  ].filter(Boolean));

  const filteredStories = initialStories.filter((s) => {
    if (topSectionIds.has(s.id)) return false;
  ...
  return (
    <div className="min-h-[100dvh] p-2 sm:p-4 md:p-6 pb-[env(safe-area-inset-bottom)] bg-surface text-charcoal font-sans">
      <div className="max-w-[1600px] mx-auto px-4 py-4">
        {/* ROW 1 (hero 4 + latest 8 = 12) and ROW 2 (main 8 + sidebar 4 = 12)
            are each their own full row — flat 12-col grid, four children,
            no nested wrapping needed since each row's spans already sum
            to exactly 12. */}
