      ...
      <table className="w-full text-left font-mono text-[11px]">
        ...
        <tbody className="divide-y divide-border/60 dark:divide-neutral-800/60">
          {standings.length > 0 ? standings.map((row) => {
            ...
            >
              <td className="py-1.5 pl-1.5 text-charcoal-soft dark:text-neutral-500">{row.pos}</td>
              <td className="py-1.5 text-charcoal dark:text-neutral-200 font-bold">{row.club}</td>
              <td className="py-1.5 text-right text-charcoal-soft dark:text-neutral-400">{row.pld}</td>
              <td className="py-1.5 text-right text-charcoal-soft dark:text-neutral-400">{row.wins}</td>
              <td className="py-1.5 text-right text-charcoal-soft dark:text-neutral-400">{row.gd}</td>
                  ...
                  {row.form.map((f, i) => (
                    <span
                      ...
                      className={`w-3.5 h-3.5 flex items-center justify-center rounded-[2px] text-[8px] font-bold ${
                        f === 'W' ? 'bg-crimson text-white' : f === 'D' ? 'bg-border dark:bg-neutral-700 text-charcoal dark:text-neutral-200' : 'bg-transparent border border-border dark:border-neutral-700 text-charcoal-soft dark:text-neutral-500'
                      }`}
                    >
                      {f}
                    </span>
            ...
            <tr><td colSpan={7} className="py-4 text-center text-charcoal-soft dark:text-neutral-500">No standings available for this tier yet.</td></tr>
          )}
        </tbody>
      </table>
      {(promotionSpots > 0 || relegationSpots > 0) && standings.length > 0 && (
        <div className="flex gap-4 text-[9px] font-mono text-charcoal-soft pt-1 border-t border-border/60 dark:border-neutral-800/60">
          {promotionSpots > 0 && (
