...
async function runForcedSync() {
  console.log('🚀 Starting Forced Master Media Vault & Ingestion Sync...');

  // 1. PROCESS TEAMS & LOGOS
  const { data: teams, error: teamErr } = await supabase.from('teams').select('id, name, slug, logo_url');
  if (teamErr) {
    console.error('❌ Error fetching teams:', teamErr.message);
    return;
  }

  console.log(`📊 Found ${teams.length} teams. Ensuring logos are synced...`);
  for (const team of teams) {
    let sourceLogo = team.logo_url;
    
    // If team has no logo URL, fallback to a reliable default badge pattern or sports CDN placeholder
    ...
    if (!sourceLogo.includes('/storage/v1/object/public/media/')) {
  ...
  const { data: players, error: playerErr } = await supabase.from('players').select('id, name, slug, metadata');
  if (playerErr) {
    console.error('❌ Error fetching players:', playerErr.message);
    return;
  }

  console.log(`📊 Found ${players.length} players. Ensuring headshots are synced...`);
  let updatedCount = 0;

  for (const player of players) {
    let rawPhoto = player.metadata?.photo;

    // If player has no photo, assign a clean professional initial fallback avatar so every card has a visual
  ...
  console.log(`🎉 Forced Media Sync Complete! Successfully updated ${updatedCount} player files and all team crests.`);
}
